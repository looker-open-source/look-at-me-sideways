...
module.exports = function ruleMatchEvaluator(ruleDef={}, project){
	...
	function shortcircuitAbstractMatch(match){
		...
		const abstractableOject = (path[2]=="view" || path[2]=="explore") && deepGet(project,path.slice(0,4))
		const isAbstract = abstractableOject
			&& abstractableOject.extension == "required"
			&& (abstractableOject.extends === undefined || abstractableOject.extends.length === 0)
		if(isAbstract && ruleDef.matchAbstract !== true){
			return {
				match,
				value: {
					level: "verbose",
