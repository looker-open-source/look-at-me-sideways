...
module.exports = function customRule(
	...
	let messages = [];
	let messageDefault = {level: 'error', rule: ruleDef.$name};
	try {
		...
		try {
			...
			console.error(e);
			throw new CodeError(`Invalid expr_rule in manifest/rule:${ruleDef.$name}`, 1);
		}
		...
		try {
			...
			throw new CodeError(`Invalid jsonpath in manifest/rule:${ruleDef.$name}/match: ${matchDef}`, 1);
		}

		messages.push({
			'level': 'info',
			'rule': ruleDef.$name,
			'description': `Rule ${ruleDef.$name} was matched ${matches.length} time(s)`,
		});
