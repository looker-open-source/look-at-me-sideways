...
async function loadManifest(
	...
	const info = (msg,loc) => messages.push({rule:"LAMS5",level:"info", description:msg,location:loc||"project"});
	const err =  (msg,loc) => messages.push({rule:"LAMS5",level:"error",description:msg,location:loc||"LAMS"});
	...
	if(manifestDefaults){
		...
		try {
			...
			info(`Default manifest settings read from ${specMeaning}`, "LAMS")
	...
	if (project.manifest) {
		...
		info(`Project manifest settings read from ${project.manifest.$file_path}`, 'manifest');
	...
	if(manifestOverrides){
		...
		try {
			...
			err(`Unable to load manifest settings from manifest override (${specMeaning}). ${e.message}.`, "LAMS")
		}
	}
