...
// CONTINUE HERE
// TODO: Need to add a function to enrich the JSON/YAML returned contents with metadata properties, e.g. `$rule`
...
function detectManifestSpecMeaning(spec){
	...
	if(spec.match(/^\s*\{\s*".*\}\s*$/)){return INLINE_JSON}
	...
	if(manifestDefaults){
		...
		try {
			switch(specMeaning){
				...
				case OBJECT: contents = spec; break;
				case INLINE_JSON: contents = JSON.parse(spec); break;
				case JSON_PATH: contents = JSON.parse(await read(spec)); break;
				case YAML_PATH: contents = maybeYamlParser.load(await read(spec));break;
