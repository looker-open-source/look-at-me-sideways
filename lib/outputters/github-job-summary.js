...
async function githubJobSummary(messages, {
	...
	const summaryTable = undent2`
		...
		${Object.values(messagesByRule)
			...
			.map(s =>
				`| [${s.rule}](#${githubAnchor(s.rule)}) | ${s.status} |${s.matches} | ${s.exempt} | ${s.errors} |`)
			.join("\n")
			}
		`
	const misc = []
...
function bulletPointMessage({globalDescription,project}){
	return function(message){
		...
		if(locationRoot === 'file'){
			const file = project.file[locationPath[1]]
			filePath = file?.$filePath
				?? locationPath[1].match(/\.(lookml|lkml)$/)
					? locationPath[1]
					: locationPath[1]+".lkml"
		...
		const locationText = message.location && message.location.replace(/[^-._\p{L}0-9\/\\: ]+/gu,"_")
		const locationLabel =
			filePath ? `**[${locationText ?? filePath}](${GITHUB_SERVER_URL}/${GITHUB_REPOSITORY}/blob/${GITHUB_SHA}/${filePath}${L})**`
			: locationText && `**${locationText}**`
		const maybeDescription = message.description && message.description !== globalDescription
			? message.description.replace(/\n/g, "\n   ")
			: ""
		return `* ${[locationLabel, maybeDescription].filter(Boolean).join(" - ")}`
		}
	}
