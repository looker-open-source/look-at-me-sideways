...
describe('Projects', () => {
	describe(testProjectName, () => {
		...
		it("it should error twice on rule maybe_string_match for model:bad", ()=> {
			expect({messages}).toContainMessage({
				rule: "maybe_string_match",
				level: "error",
				location: "model:bad/explore:bad_string"
			});
			expect({messages}).toContainMessage({
				rule: "maybe_string_match",
				level: "error",
				location: "model:bad/explore:no_string"
			});
		});
		it("it should not error on rule maybe_string_match for model:ok", ()=> {
			expect({messages}).not.toContainMessage({
				rule: "maybe_string_match",
				level: "error",
				location: "model:ok/explore:ok_string"
			});
		});
	});
});
